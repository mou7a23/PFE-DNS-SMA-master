//package Aggregation;import SWIG_generated_files.*;
...
public class Aggregation extends Robot {
	...
	public Aggregation() {
		timeStep = 256;  // set the control time step
	...
	/**
	 * Continue the robot's movement for sec secondes 
	 * Without checking the sensors
	 * @param sec
	 */
	...
	/**
	 * la methode marche aleatoire
	 */
	void randomMove(double speed){
		Random rand = new Random();
		this.move(speed, speed);
		if (rand.nextDouble() > 0.99){
			double dev = (rand.nextDouble() * speed * 2) - speed;
			this.move(Math.max(speed+dev, 100.0), speed-dev);
		}
	}
	void eviterObstacle(double[] distVal){
		if (distVal[0] > 80){
			this.move(-30, 30);
		}
	...
	* Flocking:
	 *
	* */
	void flocking(List<CameraRecognitionObject> targets, double distVal[]){
		if (distVal[0] < 80 && distVal[1] < 80 && distVal[6] < 80 && distVal[7] < 80){
			if (!targets.isEmpty()){
				double dist_min=99999;
				double distance, x = 0, y=0;
				double posPlusProche[] = new double[0];
				for(CameraRecognitionObject robot:targets){
					distance = Math.pow(robot.getPosition()[0], 2) + Math.pow(robot.getPosition()[1], 2);
					if (distance < dist_min){
						dist_min = distance;
						posPlusProche = robot.getPosition();
				...
				if(dist_min < 0.4){
					move(-x/dist_min * 100, -y/dist_min * 100);
					/** s'eloigner de target
					if (posPlusProche[1] < -0.05){
						// aller à droite
						this.move(-20, -15);
					} else if (posPlusProche[1] > 0.05) {
						// aller à gauche
						this.move(20, 15);
					}
					*/
				}else {
					// se rapprocher de target
					move(x/dist_min * 100, y/dist_min * 100);
				}
			}else {
				this.move(50, 50);
	...
	public void run() {
		...
		while (step(timeStep) != -1) {
			//this.randomMove(60);
			distVal = this.readDistanceSensorValues();
			List<CameraRecognitionObject> detected = cameraDetection();
			List<CameraRecognitionObject> targets = otherRobotsDetected(detected);

			this.flocking(targets, distVal);
	...
	public static void main(String[] args) {
		Aggregation controller = new Aggregation();
		controller.run();
	}
}